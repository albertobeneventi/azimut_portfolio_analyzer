...
def _records_to_df(records: list) -> pd.DataFrame:
    ...
    # Ricalcola sempre macro_cat + colori da gruppo (evita valori stale in cache)
...
def _mpl_macro_pie(df, wcol) -> io.BytesIO | None:
    ...
    if w_az + w_obb < 0.001:
        return None
    # Figura orizzontale: donut a sinistra, legenda testuale a destra
    ...
    # Legenda testuale nel secondo asse
    ax_leg.axis("off")
...
# ── Prior forward-looking per sottocategoria (stessa logica di optimizer.py) ─
...
def _az_portfolio_mu_sigma(
    ...
    Ritorna (mu_ptf, sigma_ptf, n_with_vol, n_total).
    """
    import numpy as np
...
def generate_pdf(df: pd.DataFrame, wcol: str, profile: str,
    ...
    macro_block = []
    if macro_buf:
        # Altezza: 8.5:3.8 aspect ratio → larghezza 14cm → altezza ~6.3cm
        _M_W = 14.0 * cm
        _M_H = _M_W * (3.8 / 8.5)
        _M_PAD = (PW - _M_W) / 2
    ...
    try:
        ...
        # Tabella: solo ±1σ (percentili 16°–84°, intervallo "normale")
    ...
    with tab2:
        ...
        with st.expander("📐 Cono di Ibbotson — Proiezione futura", expanded=False):
            ...
            with _c2:
                ...
                # Punti discreti anno per anno (stile Quantalys)
                ...
                def _lbl(arr):
                    ...
                    for i, v in enumerate(arr):
                        if int(_t_pts[i]) % _lbl_step == 0:
                            if v >= 1_000_000:
                                out.append(f"{v/1_000_000:.1f}M")
                            elif v >= 1_000:
                                out.append(f"{v/1_000:.0f}K")
                            else:
                                out.append(f"{v:.0f}")
                        else:
                            out.append("")
                ...
                _fig_ib.add_trace(go.Scatter(
                    x=_t_pts, y=_med_v,
                    mode="lines+markers+text",
                    line=dict(color="#1B4FBB", width=2.5),
                    marker=dict(size=7, color="#1B4FBB"),
                    text=_lbl(_med_v), textposition="top center",
